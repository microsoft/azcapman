...
# // T-32: RTM FR-11.2
# Check SRE Agent
$sreAgent = az resource list --resource-group $ResourceGroupName --resource-type "Microsoft.App/agents" --output json 2>$null | ConvertFrom-Json
$sreAgentResources = @($sreAgent)
$totalChecks++
if (Write-Check "SRE Agent exists" ($sreAgentResources.Count -gt 0) (($sreAgentResources | ForEach-Object { $_.name }) -join ", ")) {
    $passedChecks++

    $sreAgentProvisioned = ($sreAgentResources | Where-Object { $_.properties.provisioningState -eq "Succeeded" }).Count -eq $sreAgentResources.Count
    $sreAgentStates = ($sreAgentResources | ForEach-Object { "$($_.name): $($_.properties.provisioningState)" }) -join ", "
    $totalChecks++
    if (Write-Check "SRE Agent provisioning succeeded" $sreAgentProvisioned $sreAgentStates) {
        $passedChecks++
    }
}
